...
<%-- Import DateUtil so we can call helper methods from JSP when needed --%>
<%@ page import="com.mycompany.elearningsystem.util.DateUtil" %>
...
<head>
    ...
    <title>Dashboard — EduLearn Lecturer</title>
...
<main class="ml-56 min-h-screen p-8">
    ...
    <div class="grid grid-cols-3 gap-6">
                            ...
                            <div class="bg-white border border-gray-100 rounded-xl p-4 hover:border-gray-200 hover:shadow-sm transition cursor-pointer"
                                ...
                                <div class="flex items-center gap-3 mb-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="text-xs text-gray-400 mt-0.5">${course.studentCount} students</div>
                                    </div>
                                </div>
                                <%-- Mini stats --%>
                                <div class="grid grid-cols-3 gap-2 text-center">
                                    ...
                                    <div class="bg-gray-50 rounded-lg py-2">
                                        <div class="text-sm font-semibold text-gray-900">${course.asgnCount}</div>
                    ...
                    <c:otherwise>
                        <div class="flex flex-col gap-2">
                            <c:forEach var="sub" items="${pending_submissions}">
                                <%
                                    com.mycompany.elearningsystem.model.Submission sub =
                                        (com.mycompany.elearningsystem.model.Submission)pageContext.getAttribute("sub");
                                    %>
                                <div class="bg-white border border-gray-100 rounded-xl px-4 py-3 flex items-center gap-4
                                     ...
                                     onclick="location.href='assignment?submission_id=${sub.submissionId}'">
                                    <%-- Student avatar --%>
                                    <div class="w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center text-xs font-semibold flex-shrink-0">
                                        <%-- Initials: first 2 chars uppercased --%>
                                        <%= DateUtil.getInitials(
                                                    pageContext.findAttribute("sub") != null
                                                    ? ((com.mycompany.elearningsystem.model.Submission)
                                                            pageContext.findAttribute("sub")).getStudentName()
                                                    : ""
                                            ) %>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="text-xs text-gray-400 truncate">
                                            ${fn:escapeXml(sub.assignmentTitle)} · ${fn:escapeXml(sub.courseTitle)}
                                        </div>
                                    </div>
        ...
        <div class="flex flex-col gap-6">
                    ...
                    <c:otherwise>
                        <div class="flex flex-col gap-2">
                            <c:forEach var="attempt" items="${recent_attempts}">
                                <%--
                                    Percentage and color class must be computed per attempt.
                                    We use a scriptlet here because JSTL cannot do arithmetic
                                    on two EL variables and conditionally set a CSS class string.
                                    This is the minimal scriptlet allowed by the spec.
                                --%>
                                <%
                                    com.mycompany.elearningsystem.model.QuizAttemptResult attempt =
                                        (com.mycompany.elearningsystem.model.QuizAttemptResult) pageContext.findAttribute("attempt");
                                    int pct      = DateUtil.calcPercent(attempt.getScore(), attempt.getTotalQuestions());
                                    String pctColor = DateUtil.getPctColorClass(pct);
                                    pageContext.setAttribute("pct",      pct);
                                    pageContext.setAttribute("pctColor", pctColor);
                                %>
                                <div class="bg-white border border-gray-100 rounded-xl px-4 py-3 flex items-center gap-3">
                                    <div class="w-7 h-7 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center text-[10px] font-semibold flex-shrink-0">
                                        <%= DateUtil.getInitials(attempt.getStudentName()) %>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="text-xs font-medium text-gray-900 truncate">${fn:escapeXml(attempt.studentName)}</div>
                                        <div class="text-[10px] text-gray-400 truncate">${fn:escapeXml(attempt.quizTitle)}</div>
                                    </div>
                                    <div class="text-right flex-shrink-0">
                                        <div class="text-sm font-semibold ${pctColor}">${pct}%</div>
                                        <div class="text-[10px] text-gray-400">${attempt.score}/${attempt.totalQuestions}</div>
                                    </div>
                                </div>
                            </c:forEach>
                        </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-sm font-semibold text-gray-700">Forum activity</h2>
                    <a href="forum" class="text-xs text-gray-400 hover:text-gray-700 transition">View all →</a>
                </div>
                <c:choose>
                    <c:when test="${empty recent_posts}">
                        <div class="bg-white border border-gray-100 rounded-xl px-4 py-6 text-center">
                            <p class="text-xs text-gray-400">No forum posts yet.</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <div class="flex flex-col gap-2">
                            <c:forEach var="post" items="${recent_posts}">
                                <a href="forum?post_id=${post.id}"
                                   class="bg-white border border-gray-100 rounded-xl px-4 py-3 hover:border-gray-200 transition block">
                                    <div class="text-xs font-medium text-gray-900 truncate mb-1">
                                        ${fn:escapeXml(post.title)}
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-[10px] text-gray-400">
                                            ${fn:escapeXml(post.authorName)}
                                            <c:if test="${post.authorRole == 'lecturer'}"> · Lecturer</c:if>
                                            ·
                                            <%-- time_ago for forum post --%>
                                            <%
                                                com.mycompany.elearningsystem.model.ForumPost post =
                                                    (com.mycompany.elearningsystem.model.ForumPost) pageContext.findAttribute("post");
                                                out.print(DateUtil.timeAgo(post.getCreatedAt()));
                                            %>
                                        </span>
